...
class PropertyCompositeItemList(PropertyItem):
    def __init__(self):
        super().__init__()
    ...
    def set_data(self, data: list):
        ...
        for composite_item in composite_items:
            ...
            double_spin_box_value = QDoubleSpinBox(composite_item_widget)
            double_spin_box_value.setValue(float(composite_item[1]))
            double_spin_box_value.setMinimum(-100000000)
            double_spin_box_value.setMaximum(1000000000)
            double_spin_box_value.setButtonSymbols(QAbstractSpinBox.ButtonSymbols.NoButtons)
            double_spin_box_value.valueChanged.connect(self.change_item)
            composite_item_layout.addWidget(double_spin_box_value)
    ...
    def add_item(self):
        ...
        double_spin_box_value = QDoubleSpinBox(composite_item)
        double_spin_box_value.setValue(0.0)
        double_spin_box_value.setMinimum(-100000000)
        double_spin_box_value.setMaximum(1000000000)
        double_spin_box_value.setButtonSymbols(QAbstractSpinBox.ButtonSymbols.NoButtons)
        double_spin_box_value.valueChanged.connect(self.change_item)
    ...
    def change_item(self):
        ...
        if type(self.sender()) is QDoubleSpinBox:
            self.properties_view.apply_values_changes(('Set', [sender_index, self.sender().value()]))
