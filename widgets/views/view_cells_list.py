...
class ViewCellsList(View):
    def __init__(self):
        super().__init__()

        self.cells_list_widget = QListWidget()

        self.cells_list_widget.itemClicked.connect(self.notify_view_models_select)
        self.cells_list_widget.setContextMenuPolicy(Qt.CustomContextMenu)
        self.cells_list_widget.customContextMenuRequested.connect(self.show_context_menu)

    def notify_view_model_add(self):
        self.view_model.add_item_to_models(self.cells_list_widget.count())

    def notify_view_models_select(self):
        sender = self.cells_list_widget.sender().currentRow()
    ...
    def add_item(self, *args):
        ...
        self.cells_list_widget.insertItem(index, item_text)

    def select_item(self, item_index, item):
        self.cells_list_widget.setCurrentRow(item_index)

    def change_item(self, item):
        self.cells_list_widget.currentItem().setText(self.list_to_str(item, ' '))
